...
class Dataset:
    ...
    def __init__(self, x_users=100):
        ...
        self.State = namedlist("State", "window airco heating lightning computer") # must be mutable
        ...
        self.tasks_performed_day = x_users * [0]
    ...
    def __calculate_weight(self, row, date_dep):
        ...
        if(row.team_prompts != -1):
            ...
            # weight *= 1 + (0.1 * row.goalsetting)
            weight_before = weight
            if(row.goalsetting < 1):
                weight *= 1 + (0.1 * row.goalsetting)
            
            # if(row.team == 1):
            #     print("weight_bef=", weight_before, "row.goalsetting", row.goalsetting, "weight_aft", weight)

        

        return min(0.999, weight)
    ...
    def __generate_prompts_day(self, time, min_per_new_prompt, date_dep, 
                               prompt_goal, classification_style):
        ...
        while (time.hour < 19):
            ...
            for x in random_order:
                ...
                if(classification_style == "random"):
                    classification = random.uniform(0, 1) <= weight
                elif(classification_style == "deterministic"):
    ...
    def generate_dataset(self, period=365, min_per_new_prompt=10, date_dep=True, classification_style="random",
        ...
        classification_style : bool
            Defines how the classification is obtained. Randomly or deterministic (x >= 0.5 -> True(x)). 
            Default is random classification.

        exclude_weights : bool
            Settting this to False enables the user to include weights in the dataset. This can come in hand
            for testing purposes. Default True.
        ...
        for date in dates:
            ...
            self.tasks_performed_day = self.x_users  * [0] # Reset task count

            if (date.day == 1 and date.month == 1):
                self.team_prompts = 8 * [0, -1]

            prompts = prompts.append(self.__generate_prompts_day(date, min_per_new_prompt, 
                                                                 date_dep, prompt_goal, 
                                                                 classification_style))
        
        # prompts.to_csv("../output/full.csv", index=False)

        if(exclude_weights):
             prompts = prompts.drop(['user_weight', 'prompt_weight', 'total_weight'], axis=1)
             
        return(prompts)

    def __finish_dataset(self, df):
